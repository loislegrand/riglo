...
class UI():
    def __init__(self):
        if cmds.window('RigLoUI', exists=True):
            cmds.deleteUI('RigLoUI')
    ...
    def createUI(self):
        cmds.window('RigLoUI', t="RigLo Builder",menuBar=True, wh=(500,250),s=True)

        gblLayout = cmds.columnLayout(columnAttach=('both', 10), rowSpacing=5, columnWidth=250, adjustableColumn=True)

        cmds.menu(label="  Help  ", tearOff=False)
        cmds.menuItem(label="Reset", command=lambda *args: self.createUI())
        cmds.menuItem(label="About", command=lambda *args: self.InfoUI())
    ...
    def limbFrame(self):
        ...
        cmds.rowColumnLayout(nc=2, bgc=(.2,.2,.2), p=Cpnts)
        print(type(Cpnts))

        Lmb = Bd.subMenu(name='Limb', color=(0.3,0.5,0.3), dependencies=Cpnts)
        

        Bd.subMenu(name='Spine', color=(0.5,0.3,0.3), dependencies=Cpnts)

        Bd.subMenu(name='Tentacle', color=(0.3,0.3,0.5), dependencies=Cpnts)

        Bd.subMenu(name='Facial', color=(0.4,0.3,0.5), dependencies=Cpnts)
